...
from v1.models import User, Dashboard, Mood, DashboardMood, MoodBody
...
@v1_app.post("/user", response_model=User)
...
@v1_app.get('/dashboard', response_model=Dashboard)
...
@v1_app.get('/mood/{mood_id}', response_model=DisplayMood)
...
@v1_app.get('/mood/{mood_id}/recommendations')
...
@v1_app.post('/mood/{mood_id}/song/{song_id}')
async def add_song_to_mood(mood_id: str, song_id: str, access_token: str = Header(None, convert_underscores=False)):
    user_email = get_email(token_to_id, access_token)
    if user_email is None:
        raise HTTPException(status_code=400, detail='Failed to get user id from cache')
    curr_user = await engine.find_one(User, User.email == user_email)
...
@v1_app.delete('/mood/{mood_id}/song/{song_id}')
async def delete_song_from_mood(mood_id: str, song_id: str, access_token: str = Header(None, convert_underscores=False)):
    user_email = get_email(token_to_id, access_token)
    if user_email is None:
        raise HTTPException(status_code=400, detail='Failed to get user id from cache')
    curr_user = await engine.find_one(User, User.email == user_email)
...
@v1_app.post('/mood/{mood_id}/set_main_song/{song_id}')
async def set_main_song(mood_id: str, song_id: str, access_token: str = Header(None, convert_underscores=False)):
    user_email = get_email(token_to_id, access_token)
    if user_email is None:
        raise HTTPException(status_code=400, detail='Failed to get user id from cache')
    curr_user = await engine.find_one(User, User.email == user_email)
